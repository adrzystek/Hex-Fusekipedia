...
- https://en.wikipedia.org/wiki/Hex_(board_game)"""
    ...
    The application has been entirely written in Python (version 3.5). It makes use of standard libraries like time, urllib, re, pickle, webbrowser, bs4 and tkinter. Especially, the scraping stuff was made with urllib and bs4 packages and the whole GUI development was possible thanks to tkinter package. Stand-alone executable (the .exe file that you are running right now) was built with PyInstaller.
...
- player mode - for all sizes - collected games of a particular player.
    ...
    Default value is board size 13x13 and championship mode.
    
    Please note that the game must be finished and be at least 6 moves long in order to be collected. Also note that every game is, if needed, mirrored so that it starts in the upper-left half of the board. Changing player nickname does not negatively impact the application behaviour (no crashes, game replication) but may lead to inaccurate game descriptions (these containg who played who).
...
- "l" for the last move (">>" button)
    ...
    Adding or updating player's or championship's data is a matter of seconds (but, naturally, it depends on user's hardware and internet connection), however, updating source of Top100 players in 11x11 mode requires browsing through a bunch (exactly 100) players' htmls and, therefore, takes up at least 5 minutes (we want to be kind to the LG server, don't we?).
        ...
        about_fusekipedia_text.tag_add("bold_tag", "1.4", "1.12", "6.4", "6.7", "11.4", "11.7", "16.4", "16.11", "21.4", "21.16", "41.4", "41.11")
    ...
    def scrap_games_data(self, _source):
        ...
        try:
            ...
            self.undo_all_moves()
            if self.size == 11:
                with open("d1_11.txt", "wb") as f:
                    pickle.dump(self.d1, f)
                with open("d2_11.txt", "wb") as f:
                    pickle.dump(self.d2, f)
                with open("d3_11.txt", "wb") as f:
                    pickle.dump(self.d3, f)
                with open("d4_11.txt", "wb") as f:
                    pickle.dump(self.d4, f)
            elif self.size == 13:
                with open("d1_13.txt", "wb") as f:
                    pickle.dump(self.d1, f)
                with open("d2_13.txt", "wb") as f:
                    pickle.dump(self.d2, f)
                with open("d3_13.txt", "wb") as f:
                    pickle.dump(self.d3, f)
                with open("d4_13.txt", "wb") as f:
                    pickle.dump(self.d4, f)
            elif self.size == 19:
                with open("d1_19.txt", "wb") as f:
                    pickle.dump(self.d1, f)
                with open("d2_19.txt", "wb") as f:
                    pickle.dump(self.d2, f)
                with open("d3_19.txt", "wb") as f:
                    pickle.dump(self.d3, f)
                with open("d4_19.txt", "wb") as f:
                    pickle.dump(self.d4, f)
    ...
    def load_games_data(self):
        if self.size == 11:
            with open("d1_11.txt", "rb") as f:
                self.d1 = pickle.load(f)
            with open("d2_11.txt", "rb") as f:
                self.d2 = pickle.load(f)
            with open("d3_11.txt", "rb") as f:
                self.d3 = pickle.load(f)
            with open("d3_11.txt", "rb") as f:
                self.d3_full = pickle.load(f)   # full copy needed for technical reasons in a few places
            with open("d4_11.txt", "rb") as f:
                self.d4 = pickle.load(f)
        elif self.size == 13:
            with open("d1_13.txt", "rb") as f:
                self.d1 = pickle.load(f)
            with open("d2_13.txt", "rb") as f:
                self.d2 = pickle.load(f)
            with open("d3_13.txt", "rb") as f:
                self.d3 = pickle.load(f)
            with open("d3_13.txt", "rb") as f:
                self.d3_full = pickle.load(f)   # full copy needed for technical reasons in a few places
            with open("d4_13.txt", "rb") as f:
                self.d4 = pickle.load(f)
        elif self.size == 19:
            with open("d1_19.txt", "rb") as f:
                self.d1 = pickle.load(f)
            with open("d2_19.txt", "rb") as f:
                self.d2 = pickle.load(f)
            with open("d3_19.txt", "rb") as f:
                self.d3 = pickle.load(f)
            with open("d3_19.txt", "rb") as f:
                self.d3_full = pickle.load(f)   # full copy needed for technical reasons in a few places
            with open("d4_19.txt", "rb") as f:
                self.d4 = pickle.load(f)
    ...
    def prepare_dicts(self, conditional=False):
        ...
        #self.d3 --> KEY - game id, VALUE - [red player, blue player, red won flag, event, [matching]]
        ...
        if conditional and self.recent_games_entry.get().strip(): # hasattr(self, "recent_games_entry")
            list_of_game_ids.sort(key=lambda x: int(x[0]))
    ...
    def arrange_next_moves(self, only_technical=False):
        ...
        for n, move in enumerate(moves_list):
            ...
            if move[0] != "swap" and len(moves_list) > 1:    # highlighting (phantoms) only makes sense with more than one move to highlight (phantom)
